...
class DiscogsHandler:
    ...
    def get_release_data(self, release_id: str, query: str):
        """Get release data with timing measurement - NOW WITH PRICING"""
        start_time = time.time()
        logger.info(f"Discogs API CALL [START]: Combined release data for {release_id}")
        
        # Get basic release info
        release_info = self._get_basic_release_info(release_id)
        image_url = release_info.get('image_url', '')
        
        # Get pricing data (will use cache if available)
        pricing_data = self.get_release_statistics_pricing(release_id)
        
        duration = time.time() - start_time
        logger.info(f"Discogs API CALL [END]: Combined release data for {release_id} - {duration:.3f}s")
    ...
    def _get_basic_release_info(self, release_id: str):
        ...
        if response.status_code == 200:
            ...
            return {'image_url': '', 'release_data': {}, 'api_time': duration}
    ...
    def get_release_tracklist(self, release_id: str):
        ...
        start_time = time.time()
        logger.info(f"Discogs API CALL [START]: GET /releases/{release_id} (tracklist)")
        ...
        duration = time.time() - start_time
        logger.info(f"Discogs API CALL [END]: GET /releases/{release_id} (tracklist) - {duration:.3f}s - Status: {response.status_code}")
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Failed to get tracklist: {response.status_code}")
            return []
    ...
    def get_simple_search_results(self, query: str, filename_base: str = None):
        ...
        start_time = time.time()
        logger.info(f"Discogs API CALL [START]: GET /database/search (simple) - q={query[:50]}...")
        ...
        duration = time.time() - start_time
        logger.info(f"Discogs API CALL [END]: GET /database/search (simple) - q={query[:50]}... - {duration:.3f}s - Status: {response.status_code}")
