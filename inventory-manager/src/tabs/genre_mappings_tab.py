...
class GenreMappingsTab:
    ...
    def _process_import_data_fast(self, import_df):
        ...
        # Use API for genre operations
        # Step 1: Get all existing genres
        existing_genres_df = st.session_state.db_manager.get_all_genres()
        existing_genres = {row['genre_name']: row['id'] for _, row in existing_genres_df.iterrows()}
        ...
        # Step 3: Bulk create new genres using API
        if genres_to_create:
            status_text.text("Creating new genres...")
            for i, genre_name in enumerate(genres_to_create):
                success, new_genre_id = st.session_state.db_manager.add_genre(genre_name)
                if success:
                    existing_genres[genre_name] = new_genre_id
                progress_bar.progress((i + 1) / (len(genres_to_create) + total_rows) * 0.3)
        
        # Step 4: Bulk assign genres to artists using API
        ...
        for i, (artist, genre_name) in enumerate(valid_rows):
            ...
            # Use API to assign genre to artist
    ...
    def _remove_unused_genres(self):
        """Remove genres that are not assigned to any artists using API"""
        # This would require a new API endpoint to remove unused genres
        # For now, return 0 as we don't have this API endpoint
        st.warning("Remove unused genres functionality requires API endpoint implementation")
        return 0
