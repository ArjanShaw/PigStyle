...
class PriceTagTab:
    def __init__(self):
        # Remove db_manager dependency and use direct API calls
        from handlers.price_tag_handler import PriceTagHandler
        self.price_tag_handler = PriceTagHandler()
    ...
    def render(self):
        ...
        # Get all records via API for count
        try:
            ...
            st.error(f"Error getting record counts: {e}")
            printed_count = 0
            total_count = 0
                        ...
                        if response.status_code == 200:
                            ...
                            if st.button(f"CONFIRM: Clear ALL {clear_count} barcodes", type="primary"):
                                ...
                                for record in records_with_barcodes:
                                    requests.put(f"https://arjanshaw.pythonanywhere.com/records/{record['id']}", 
                                                json={'barcode': None})
    ...
    def _print_tags(self, record_ids):
        ...
        # Get records via API
        try:
            response = requests.post("https://arjanshaw.pythonanywhere.com/records/by-ids",
