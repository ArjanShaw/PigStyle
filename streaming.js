...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    setTimeout(loadRecordsFromAPI, 500);// streaming.js - Get genres from records, random start, YouTube only with upvotes only
// FIXED VOTE REFRESH ISSUE
...
class VotingSystem {
    ...
    // NEW: Fetch fresh vote count from API
    async fetchFreshVoteCount(recordId) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/record-votes/${recordId}`);
            if (response.ok) {
                const data = await response.json();
        ...
        try {
            ...
            if (data && data.status === 'success') {
                ...
                if (filteredRecords.length > 0 && currentTrackIndex < filteredRecords.length) {
                    const record = filteredRecords[currentTrackIndex];
                    if (record.id === recordId) {
                        // Use fresh count from API or increment locally
                        if (freshCount !== null) {
                            record.up_votes = freshCount;
                        } else {
